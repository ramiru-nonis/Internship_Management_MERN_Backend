...
exports.generateStudentReport = async (req, res) => {
    try {
        ...
        return generatePDF(res, student, placement, logbooks);
...
// Generate Full Student Report (Public/Mentor Context via Logbook ID)
exports.generateStudentReportForLogbook = async (req, res) => {
    try {
        const { id } = req.params; // Logbook ID

        // 1. Find Logbook to get Student ID
        const logbook = await Logbook.findById(id);
        if (!logbook) return res.status(404).json({ message: "Logbook not found" });

        const studentId = logbook.studentId;
        ...
        console.error("Error generating report via logbook:", error);
        res.status(500).json({ message: "Failed to generate report", error: error.message });
    }
};

const generatePDF = (res, student, placement, logbooks) => {
    // 2. Init PDF
